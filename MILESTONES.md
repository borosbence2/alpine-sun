...
| A5.use.3   | CMake auto-fetches one JPEG of ESRI World Imagery per preset's bbox at configure time (`assets/sat/{preset}.jpg`, ~200–300 KB). Loaded via stb_image at startup (declarations only — forfun_core's tinygltf_impl already provides the implementation), uploaded as `R8G8B8A8_SRGB` so the sampler does gamma. New descriptor binding 4 + `satParams` UBO field. `terrain.frag` overrides the procedural grass/rock/snow ramp with the satellite texture when the user enables "Satellite albedo" in the Sun panel. Toggle is disabled for `matterhorn-wide` (no bundled image). | DONE |
| A5.use.4   | Hillaire 2020 §5.1+5.3 (subset): transmittance LUT (256×64 RGBA16F, one-shot at startup) + sky-view LUT (192×108 RGBA16F, rebaked when sun direction drifts >0.5°). Single-scattering only — no multi-scattering, no aerial perspective. `sky.frag` samples the sky-view LUT and adds a small sun disk; same ACES tonemap as terrain. New binding 5 on the camera descriptor set. | DONE |
